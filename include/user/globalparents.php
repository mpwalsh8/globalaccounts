...
require_once("globalparents.forms.class.php") ;
 ...
 * @see Container
 */
class GlobalParentsTabContainer extends Container
{
    /**
    ...
    function __buildGuidance()
    {
        ...
        $ul->add(html_li(html_b(__(WPGA_ACTION_ADD)),
            __(":  Add a Global Parent."))) ;
        $ul->add(html_li(html_b(__(WPGA_ACTION_UPDATE)),
            __(":  Update a Global Parent."))) ;
        $ul->add(html_li(html_b(__(WPGA_ACTION_DELETE)),
            __(":  Delete a Global Parent."))) ;
    ...
    function __buildGDL()
    {
        $gdl = new GlobalAccountsGlobalParentsGUIDataList("Global Parents",
            "75%", "globalparentname", false) ;
    ...
    /**
     * Construct the content of the GlobalParents Tab Container
     */
    function GlobalParentsTabContainer()
    {
        ...
        $div = html_div() ;
        $div->add(html_h2("Global Parents"), html_br()) ;

        //  This allows passing arguments eithers as a GET or a POST

        $scriptargs = array_merge($_GET, $_POST) ;

        //  The globalparentid is the argument which must be
        //  dealt with differently for GET and POST operations

        if (array_key_exists(WPGA_DB_PREFIX . "radio", $scriptargs))
            $globalparentid = $scriptargs[WPGA_DB_PREFIX . "radio"][0] ;
        else if (array_key_exists("globalparentid", $scriptargs))
            $globalparentid = $scriptargs["globalparentid"] ;
        else
            $globalparentid = null ;
        ...
        else  //  Crank up the form processing process
        {
            switch ($action)
            {
                case WPGA_ACTION_ADD:
                    $form = new GlobalAccountsGlobalParentAddForm("Add GlobalParent",
                        $_SERVER['HTTP_REFERER'], 600) ;
                    break ;

                case WPGA_ACTION_UPDATE:
                    $form = new GlobalAccountsGlobalParentUpdateForm("Update GlobalParent",
                        $_SERVER['HTTP_REFERER'], 600) ;
                    $form->setGlobalParentId($globalparentid) ;
                    break ;

                case WPGA_ACTION_DELETE:
                    $form = new GlobalAccountsGlobalParentDeleteForm("Delete GlobalParent",
                        $_SERVER['HTTP_REFERER'], 600) ;
                    $form->setGlobalParentId($globalparentid) ;
                    break ;
 ...
 * @see Container
 */
class AdminGlobalParentsTabContainer extends GlobalParentsTabContainer
{
    /**
     * Build the GUI DataList used to display the roster
     *
    ...
    function __buildGDL()
    {
        $gdl = new GlobalAccountsGlobalParentsAdminGUIDataList("Global Parents",
            "75%", "globalparentname", false) ;
