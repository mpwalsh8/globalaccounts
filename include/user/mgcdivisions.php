...
require_once("mgcdivisions.forms.class.php") ;
 ...
 * @see Container
 */
class MGCDivisionsTabContainer extends Container
{
    /**
    ...
    function __buildGDL()
    {
        $gdl = new GlobalAccountsMGCDivisionsGUIDataList("MGC Divisions",
            "800px", "mgcdivisionshortname", false) ;
    ...
    /**
     * Construct the content of the MGCDivisions Tab Container
     */
    function MGCDivisionsTabContainer()
    {
        ...
        $div = html_div() ;
        $div->add(html_h2("MGC Divisions"), html_br()) ;

        //  This allows passing arguments eithers as a GET or a POST

        $scriptargs = array_merge($_GET, $_POST) ;

        //  The mgcdivisionid is the argument which must be
        //  dealt with differently for GET and POST operations

        if (array_key_exists(WPGA_DB_PREFIX . "radio", $scriptargs))
            $mgcdivisionid = $scriptargs[WPGA_DB_PREFIX . "radio"][0] ;
        else if (array_key_exists("mgcdivisionid", $scriptargs))
            $mgcdivisionid = $scriptargs["mgcdivisionid"] ;
        else
            $mgcdivisionid = null ;
        ...
        else  //  Crank up the form processing process
        {
            switch ($action)
            {
                case WPGA_ACTION_ADD:
                    $form = new GlobalAccountsMGCDivisionAddForm("Add MGCDivision",
                        $_SERVER['HTTP_REFERER'], 600) ;
                    break ;

                case WPGA_ACTION_UPDATE:
                    $form = new GlobalAccountsMGCDivisionUpdateForm("Update MGCDivision",
                        $_SERVER['HTTP_REFERER'], 600) ;
                    $form->setMGCDivisionId($mgcdivisionid) ;
                    break ;

                case WPGA_ACTION_DELETE:
                    $form = new GlobalAccountsMGCDivisionDeleteForm("Delete MGCDivision",
                        $_SERVER['HTTP_REFERER'], 600) ;
                    $form->setMGCDivisionId($mgcdivisionid) ;
                    break ;
 ...
 * @see Container
 */
class AdminMGCDivisionsTabContainer extends MGCDivisionsTabContainer
{
    /**
     * Build the GUI DataList used to display the roster
     *
    ...
    function __buildGDL()
    {
        $gdl = new GlobalAccountsMGCDivisionsAdminGUIDataList("MGC Divisions",
            "800px", "mgcdivisionshortname", false) ;
